...
QtMaterialFlatButtonStateMachine::QtMaterialFlatButtonStateMachine(
    ...
    : QStateMachine(parent), m_button(parent),
      ...
      m_haloAnimation(new QSequentialAnimationGroup(this)), m_overlayOpacity(0),
      m_checkedOverlayProgress(parent->isChecked() ? 1 : 0), m_haloOpacity(0),
      m_haloSize(0.8), m_haloScaleFactor(1), m_wasChecked(false) {
  Q_ASSERT(parent);
  ...
  addTransition(m_button, QEvent::FocusIn, m_neutralState,
                m_neutralFocusedState);
  addTransition(m_button, QEvent::FocusOut, m_neutralFocusedState,
                m_neutralState);
  ...
  addTransition(m_button, QEvent::Enter, m_neutralFocusedState,
                m_hoveredFocusedState);
  addTransition(m_button, QEvent::Leave, m_hoveredFocusedState,
                m_neutralFocusedState);
  addTransition(m_button, QEvent::FocusIn, m_hoveredState,
                m_hoveredFocusedState);
  addTransition(m_button, QEvent::FocusOut, m_hoveredFocusedState,
                m_hoveredState);
...
void QtMaterialFlatButtonStateMachine::setCheckedOverlayProgress(
    qreal progress) {
...
void QtMaterialFlatButtonStateMachine::updateCheckedStatus() {
  ...
  if (m_wasChecked != checked) {
    ...
    if (checked) {
      postEvent(
          new QtMaterialStateTransitionEvent(FlatButtonCheckedTransition));
    } else {
      postEvent(
          new QtMaterialStateTransitionEvent(FlatButtonUncheckedTransition));
    }
  }
}
...
bool QtMaterialFlatButtonStateMachine::eventFilter(QObject *watched,
  ...
  if (QEvent::FocusIn == event->type()) {
    ...
    if (focusEvent && Qt::MouseFocusReason == focusEvent->reason()) {
      postEvent(
          new QtMaterialStateTransitionEvent(FlatButtonPressedTransition));
      return true;
    }
  }
...
void QtMaterialFlatButtonStateMachine::addTransition(
    QAbstractTransition *transition, QState *fromState, QState *toState) {
