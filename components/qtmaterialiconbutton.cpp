...
QtMaterialIconButton::QtMaterialIconButton(const QIcon &icon, QWidget *parent)
    : QAbstractButton(parent), d_ptr(new QtMaterialIconButtonPrivate(this)) {
  d_func()->init();
...
QtMaterialIconButton::QtMaterialIconButton(QtMaterialIconButtonPrivate &d,
                                           QWidget *parent)
...
void QtMaterialIconButton::mousePressEvent(QMouseEvent *event) {
  ...
  d->rippleOverlay->addRipple(
      QPoint(d->rippleOverlay->width(), d->rippleOverlay->height()) / 2,
      iconSize().width());
  emit clicked();
...
void QtMaterialIconButton::paintEvent(QPaintEvent *event) {
  ...
  painter.drawPixmap(QRect((r.width() - w) / 2, (r.height() - h) / 2, w, h),
                     pixmap);
}
