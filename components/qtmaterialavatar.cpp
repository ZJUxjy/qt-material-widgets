...
QtMaterialAvatar::QtMaterialAvatar(QWidget *parent)
    : QWidget(parent), d_ptr(new QtMaterialAvatarPrivate(this)) {
  d_func()->init();
}
...
void QtMaterialAvatar::setSize(int size) {
  ...
  if (!d->image.isNull()) {
    d->pixmap = QPixmap::fromImage(d->image.scaled(
        d->size, d->size, Qt::IgnoreAspectRatio, Qt::SmoothTransformation));
  }
...
void QtMaterialAvatar::setImage(const QImage &image) {
  ...
  d->pixmap = QPixmap::fromImage(image.scaled(
      d->size, d->size, Qt::IgnoreAspectRatio, Qt::SmoothTransformation));
  update();
}
...
void QtMaterialAvatar::paintEvent(QPaintEvent *event) {
  ...
  case Material::ImageAvatar: {
    ...
    painter.drawPixmap(
        QRect(width() / 2 - hs, height() / 2 - hs, d->size, d->size),
        d->pixmap);
    break;
  }
