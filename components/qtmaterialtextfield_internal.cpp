...
QtMaterialTextFieldStateMachine::QtMaterialTextFieldStateMachine(
    ...
    : QStateMachine(parent), m_textField(parent), m_normalState(new QState),
      m_focusedState(new QState), m_label(0), m_offsetAnimation(0),
      m_colorAnimation(0), m_progress(0.0) {
  Q_ASSERT(parent);
  ...
  connect(m_textField, SIGNAL(textChanged(QString)), this,
          SLOT(setupProperties()));
}

QtMaterialTextFieldStateMachine::~QtMaterialTextFieldStateMachine() {}

void QtMaterialTextFieldStateMachine::setLabel(
...
void QtMaterialTextFieldStateMachine::setupProperties() {
  if (m_label) {
    ...
    } else if (!m_textField->hasFocus() && m_label->offset().y() <= 0 &&
               m_textField->text().isEmpty()) {
...
QtMaterialTextFieldLabel::QtMaterialTextFieldLabel(QtMaterialTextField *parent)
    : QWidget(parent), m_textField(parent), m_scale(1), m_posX(0), m_posY(26),
