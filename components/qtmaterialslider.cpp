...
QRectF QtMaterialSliderPrivate::thumbBoundingRect() const {
  ...
  return Qt::Horizontal == q->orientation()
             ? QRectF(
                   thumb->offset(), q->height() / 2 - QT_MATERIAL_SLIDER_MARGIN,
                   QT_MATERIAL_SLIDER_MARGIN * 2, QT_MATERIAL_SLIDER_MARGIN * 2)
             : QRectF(q->width() / 2 - QT_MATERIAL_SLIDER_MARGIN,
                      thumb->offset(), QT_MATERIAL_SLIDER_MARGIN * 2,
...
int QtMaterialSliderPrivate::valueFromPosition(const QPoint &pos) const {
  ...
  return QtMaterialStyle::sliderValueFromPosition(
      q->minimum(), q->maximum(), position - QT_MATERIAL_SLIDER_MARGIN, span,
  ...
  if (!q->hasFocus()) {
    if (status) {
      stateMachine->postEvent(
          new QtMaterialStateTransitionEvent(SliderNoFocusMouseEnter));
    } else {
      stateMachine->postEvent(
          new QtMaterialStateTransitionEvent(SliderNoFocusMouseLeave));
    }
  }
...
QtMaterialSlider::QtMaterialSlider(QWidget *parent)
    : QAbstractSlider(parent), d_ptr(new QtMaterialSliderPrivate(this)) {
  d_func()->init();
}
  ...
  if (SliderOrientationChange == change) {
    ...
    if (minimum() == value()) {
      ...
      d->stateMachine->postEvent(
          new QtMaterialStateTransitionEvent(SliderChangedToMinimum));
    } else if (maximum() == value()) {
      triggerAction(SliderToMaximum);
    }
    if (minimum() == d->oldValue) {
      d->stateMachine->postEvent(
          new QtMaterialStateTransitionEvent(SliderChangedFromMinimum));
    }
    d->oldValue = value();
  }
...
void QtMaterialSlider::mousePressEvent(QMouseEvent *event) {
  ...
  SliderAction action =
      d->stepTo > sliderPosition() ? SliderPageStepAdd : SliderPageStepSub;
