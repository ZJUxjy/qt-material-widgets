...
DrawerSettingsEditor::DrawerSettingsEditor(QWidget *parent)
    : QWidget(parent), ui(new Ui::DrawerSettingsForm),
  ...
  QVector<QString> labels = {
      "Motion",     "Style",     "Layout",
      "Components", "Patterns",  "Growth & communications",
      "Usability",  "Platforms", "Resources"};
  ...
  connect(ui->showDrawerButton, SIGNAL(pressed()), m_drawer,
          SLOT(openDrawer()));
  connect(ui->hideDrawerButton, SIGNAL(pressed()), m_drawer,
          SLOT(closeDrawer()));
  connect(ui->clickToCloseCheckBox, SIGNAL(toggled(bool)), this,
          SLOT(updateWidget()));
  connect(ui->overlayModeCheckBox, SIGNAL(toggled(bool)), this,
          SLOT(updateWidget()));
}

DrawerSettingsEditor::~DrawerSettingsEditor() { delete ui; }
