...
DialogSettingsEditor::DialogSettingsEditor(QWidget *parent)
    : QWidget(parent), ui(new Ui::DialogSettingsForm),
  ...
  dialogWidgetLayout->setAlignment(closeButton,
  ...
  connect(ui->showDialogButton, SIGNAL(pressed()), m_dialog,
          SLOT(showDialog()));
  connect(closeButton, SIGNAL(pressed()), m_dialog, SLOT(hideDialog()));
}

DialogSettingsEditor::~DialogSettingsEditor() { delete ui; }
