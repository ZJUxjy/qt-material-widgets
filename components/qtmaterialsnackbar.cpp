...
qreal QtMaterialSnackbar::fontSize() const { return font().pointSizeF(); }
...
void QtMaterialSnackbar::addMessage(const QString &message) {
  ...
  d->stateMachine->postEvent(
      new QtMaterialStateTransitionEvent(SnackbarShowTransition));
  raise();
}
...
void QtMaterialSnackbar::dequeue() {
  ...
  if (!d->messages.isEmpty()) {
    d->stateMachine->postEvent(
        new QtMaterialStateTransitionEvent(SnackbarNextTransition));
  } else {
    d->stateMachine->postEvent(
        new QtMaterialStateTransitionEvent(SnackbarWaitTransition));
  }
}
...
void QtMaterialSnackbar::paintEvent(QPaintEvent *event) {
  ...
  painter.drawText(br, Qt::AlignHCenter | Qt::AlignTop | Qt::TextWordWrap,
                   message);
}
