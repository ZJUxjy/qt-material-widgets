...
QtMaterialFlatButton::QtMaterialFlatButton(QWidget *parent,
    ...
    : QPushButton(parent), d_ptr(new QtMaterialFlatButtonPrivate(this)) {
  d_func()->init();

  applyPreset(preset);
}

QtMaterialFlatButton::QtMaterialFlatButton(const QString &text, QWidget *parent,
                                           Material::ButtonPreset preset)
    : QPushButton(text, parent), d_ptr(new QtMaterialFlatButtonPrivate(this)) {
  d_func()->init();
...
QtMaterialFlatButton::QtMaterialFlatButton(const QString &text,
                                           Material::Role role, QWidget *parent,
                                           Material::ButtonPreset preset)
    : QPushButton(text, parent), d_ptr(new QtMaterialFlatButtonPrivate(this)) {
  d_func()->init();
...
void QtMaterialFlatButton::setIconPlacement(
    Material::ButtonIconPlacement placement) {
  Q_D(QtMaterialFlatButton);
...
QtMaterialFlatButton::QtMaterialFlatButton(QtMaterialFlatButtonPrivate &d,
                                           QWidget *parent,
                                           Material::ButtonPreset preset)
...
void QtMaterialFlatButton::paintHalo(QPainter *painter) {
  ...
  const qreal s =
      d->stateMachine->haloScaleFactor() * d->stateMachine->haloSize();
...
void QtMaterialFlatButton::paintForeground(QPainter *painter) {
  ...
  if (isEnabled()) {
    ...
    if (isCheckable() && progress > 0) {
      ...
      QColor dest =
          Qt::TransparentMode == d->bgMode ? Qt::white : backgroundColor();
  ...
  QPoint pos(Qt::AlignLeft == d->textAlignment ? 12 : (base.width() - iw) / 2,
             0);

  QRect textGeometry(pos + QPoint(0, base.height() / 2), textSize);
  QRect iconGeometry(pos + QPoint(0, (height() - iconSize().height()) / 2),
                     iconSize());
