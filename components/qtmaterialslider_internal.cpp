...
QtMaterialSliderStateMachine::QtMaterialSliderStateMachine(
    QtMaterialSlider *slider, QtMaterialSliderThumb *thumb,
    QtMaterialSliderTrack *track)
    : QStateMachine(slider), m_slider(slider), m_thumb(thumb), m_track(track),
      m_topState(new QState(QState::ParallelStates)),
      m_fstState(new QState(m_topState)), m_sndState(new QState(m_topState)),
  ...
  customTransition->addAnimation(
      new QPropertyAnimation(track, "fillColor", this));
  ...
  transition =
      new QSignalTransition(m_pulseOutState, SIGNAL(propertiesAssigned()));
  ...
  transition =
      new QSignalTransition(m_pulseInState, SIGNAL(propertiesAssigned()));
...
void QtMaterialSliderStateMachine::setupProperties() {
  ...
  m_inactiveState->assignProperty(m_track, "fillColor",
                                  trackColor.lighter(130));
  ...
  if (m_slider->parentWidget()) {
    holeColor =
        m_slider->parentWidget()->palette().color(m_slider->backgroundRole());
  }
...
QtMaterialSliderThumb::QtMaterialSliderThumb(QtMaterialSlider *slider)
    : QtMaterialOverlayWidget(slider->parentWidget()), m_slider(slider),
      m_diameter(11), m_borderWidth(2), m_haloSize(0), m_offset(0) {
...
void QtMaterialSliderThumb::paintEvent(QPaintEvent *event) {
  ...
  QPointF disp = Qt::Horizontal == m_slider->orientation()
                     ? QPointF(QT_MATERIAL_SLIDER_MARGIN + m_offset,
                               m_slider->height() / 2)
                     : QPointF(m_slider->width() / 2,
                               QT_MATERIAL_SLIDER_MARGIN + m_offset);
  ...
  brush.setColor(m_slider->isEnabled() ? m_fillColor
  ...
  QRectF geometry =
      Qt::Horizontal == m_slider->orientation()
...
QtMaterialSliderTrack::QtMaterialSliderTrack(QtMaterialSliderThumb *thumb,
                                             QtMaterialSlider *slider)
    : QtMaterialOverlayWidget(slider->parentWidget()), m_slider(slider),
      m_thumb(thumb), m_trackWidth(2) {
  slider->installEventFilter(this);
...
void QtMaterialSliderTrack::paintEvent(QPaintEvent *event) {
  ...
  QRectF geometry =
      Qt::Horizontal == m_slider->orientation()
          ? QRectF(0, 0, m_slider->width() - QT_MATERIAL_SLIDER_MARGIN * 2,
                   m_trackWidth)
          : QRectF(0, 0, m_trackWidth,
                   m_slider->height() - QT_MATERIAL_SLIDER_MARGIN * 2);
  ...
  if (Qt::Horizontal == m_slider->orientation()) {
    ...
    bgRect = QRectF(offset, 0, m_slider->width(), m_trackWidth)
                 .intersected(geometry);
  } else {
    fgRect = QRectF(0, 0, m_trackWidth, offset);
    bgRect = QRectF(0, offset, m_trackWidth, m_slider->height())
                 .intersected(geometry);
  }
